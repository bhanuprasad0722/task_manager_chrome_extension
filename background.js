...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (message.type === 'createAlarm') {
        ...
        if (timeUntilDeadline > 0) {
            for (let i = 1; i <= 3; i++) {
                chrome.alarms.create(`reminder-${task.id}-${i}`, {
    ...
    if (message.type === 'removeAlarm') {
        chrome.alarms.clear(`reminder-${message.taskId}-1`);
        chrome.alarms.clear(`reminder-${message.taskId}-2`);
        chrome.alarms.clear(`reminder-${message.taskId}-3`);
    }
    ...
    if (alarm.name.startsWith('reminder-')) {
        ...
        chrome.storage.local.get(['tasks'], function (data) {
            ...
            if (task) {
                ...
                // Move task to pending after the first reminder
                if (reminderCount === '1') {
...
function moveTaskToPending(task) {
    chrome.storage.local.get(['tasks', 'pendingTasks'], function (data) {
        ...
        // Notify the popup to refresh the lists
